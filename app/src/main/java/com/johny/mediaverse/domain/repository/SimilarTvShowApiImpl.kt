...
class SimilarTvShowApiImpl(
    ...
    override suspend fun getPagedTvShow(tvShowId: Int, page: Int): Result<TvShowResponseDto, NetworkError> {
        return safeCall<TvShowResponseDto> {
            httpClient.get(
                urlString = BuildConfig.MOVIE_DB_BASE_URL + Constants.MovieDbUrl.TV_DETAILS + "$tvShowId" + Constants.MovieDbUrl.SIMILAR_TV_SHOW
            ) {
                headers.append(
